...
#
# TODO
#    1) Add table pattern matching (ie. e_*,x_* )
#    2) Create file subdirectories as needed to write to ODS.
#
# AUTHOR
#	Caleb Grant (CG)
#
...
class Database():
    ...
    def open_db(self):
        ...
        if self.password is not None:
            try:
                self.conn = psycopg2.connect(f"host='{self.host}' dbname='{self.db}' user='{self.user}' password='{self.password}'")
    ...
    def __repr__(self):
        return u"Database(host=%r, database=%r, user=%r)" % (self.host, self.db, self.user)
...
class DatabaseQuery(Database):
    ...
    def table_list(self):
        ...
        if self.tbls is None:
            tables = self.execute(f"SELECT table_name FROM information_schema.tables WHERE table_schema = '{self.schema}' ORDER BY table_name;")
            self.tbl_list = [table[0] for table in tables]
        else:
            try:
                tables = [t for t in self.tbls.split(',')]
                for tbl in tables:
                    if self.verify_tbl(tbl):
                        self.tbl_list.append(tbl)
                    else:
                        pass
            except Exception as err:
                raise err
    ...
    def verify_tbl(self, tbl):
        c = self.execute(f"SELECT * FROM information_schema.tables WHERE table_name = '{tbl}';")
    ...
    def column_list(self, tbl):
        cols = self.execute(f"SELECT column_name FROM information_schema.columns WHERE table_schema = '{self.schema}' AND table_name = '{tbl}';")
...
def clparser():
    ...
    desc_msg = """Create an ODF file of empty data tables with
        matching columns to the specified database tables. Version %s, %s""" % (__version__, __vdate)
    parser = argparse.ArgumentParser(description=desc_msg)
    parser.add_argument('output_file', 
        help="Name of the output file containing data table templates")
    ...
    parser.add_argument('-t', '--tables', type=str, dest='table_list',
        help="""Comma seperated list of tables for template creation. If no tables are specified, all schema tables will be used.""")
    parser.add_argument('-x', '--columns', type=str, dest='x_columns', 
        help="""Comma seperated list of columns that should not be included in the output table templates.""")
    return parser
